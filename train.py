...
def train():
    t = Train(epochs=3, n_min=1e-7, n_max=4e-5, path_to_model="model_v5.h5")
    t.train(name="model_v5_2.h5")


def fine_tune():
    fine_tune = Train(epochs=4, n_min=1e-8, n_max=1e-6)
    fine_tune.train(name="model_v5_fine_tuned.h5", fine_tune=True)
